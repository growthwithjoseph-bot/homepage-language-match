// Radial coverage-map generator — ported from reference/radial-map.js.
...
function renderRadialMap(container, map, onTopicClick) {
  ...
  cats.forEach((cat) => {
    ...
    (cat.topics || []).forEach((t) => {
      ...
      leafDots += `<circle class="tc-node" data-id="${t.id}" cx="${lx.toFixed(1)}" cy="${ly.toFixed(1)}" r="5.5" fill="${color}" stroke="#fff" stroke-width="1.2" style="cursor:pointer;"><title>${escapeXml(t.label)} — ${t.state} (you ${t.you_pct}%)</title></circle>`;
      labels += `<text class="tc-label" data-id="${t.id}" x="${tx.toFixed(1)}" y="${ty.toFixed(1)}" transform="rotate(${rot.toFixed(1)} ${tx.toFixed(1)} ${ty.toFixed(1)})" text-anchor="${anchor}" dominant-baseline="middle" font-size="10.5" fill="#475569" style="cursor:pointer;">${escapeXml(t.label)}</text>`;
    });
    ...
    angs.forEach((ang) => {
      ...
      edges += `<path d="M ${ccx.toFixed(1)} ${ccy.toFixed(1)} Q ${mx.toFixed(1)} ${my.toFixed(1)} ${lx.toFixed(1)} ${ly.toFixed(1)}" fill="none" stroke="#cbd5e1" stroke-opacity="0.5" stroke-width="0.9"/>`;
    });
    catNodes += `<circle cx="${ccx.toFixed(1)}" cy="${ccy.toFixed(1)}" r="6.5" fill="${CATEGORY_COLOR}"/>`;
    const offY = Math.sin(catAng) < 0 ? -12 : 18;
    catNodes += `<text x="${ccx.toFixed(1)}" y="${(ccy + offY).toFixed(1)}" text-anchor="middle" font-size="12" font-weight="700" fill="#334155" style="pointer-events:none;">${escapeXml(cat.label)}</text>`;
    slot += gapSlots;
  });

  const root = (map.own_domain || 'domain').split('.')[0].toUpperCase();
  const rootSvg = `<circle cx="${cx}" cy="${cy}" r="34" fill="#475569"/>` +
    `<text x="${cx}" y="${cy - 2}" text-anchor="middle" font-size="12" font-weight="800" fill="#fff" style="pointer-events:none;">${escapeXml(root)}</text>` +
  ...
  if (onTopicClick) {
    container.querySelectorAll('.tc-node, .tc-label').forEach(el => {
      el.addEventListener('click', () => onTopicClick(String(el.getAttribute('data-id'))));
    });
  }
}
...
// Highlight the currently-selected topic's node + label.
function highlightSelected(container, topicId) {
  container.querySelectorAll('.tc-node').forEach(el => {
    const on = el.getAttribute('data-id') === String(topicId);
    el.setAttribute('r', on ? '8.5' : '5.5');
    el.setAttribute('stroke', on ? '#0f172a' : '#fff');
    el.setAttribute('stroke-width', on ? '2' : '1.2');
  });
  container.querySelectorAll('.tc-label').forEach(el => {
    const on = el.getAttribute('data-id') === String(topicId);
    el.setAttribute('font-weight', on ? '700' : '400');
    el.setAttribute('fill', on ? '#0f172a' : '#475569');
  });
}
